...
/*!
 * 
 * (c) 2009-2010 Torstein Hønsi and FusionCharts Technologies LLP.
 *
...
var doc = document,
	...
	seriesTypes = {},
        ...
        // constants.
        ADJUSTZERO_NUDGE = 0.5;





///function to create rgba color format
...
function drawArrow (arr, X1, Y1, X2, Y2, R, H) {
    var tanganent = (Y1 - Y2) / (X1 - X2), angle = math.atan(tanganent), PX, PY, RHlf, HHlf;
 ...
 *
 * Function has been modified at FusionCharts Lab for performance optimization.
 */
function pick() {
	var arg, i, l;
...
/**
 * Return the first value that is defined. Like MooTools' $.pick. modyfied for FC
 */
function pluck() {
    var arg, i, l;
    for (i = 0, l = arguments.length; i < l; i += 1) {
...
/**
 * Function to address situations where we do not accept zero.
 */
function adjustZero(val) {
    return val == 0 && val !== false && val !== '' ? ADJUSTZERO_NUDGE : val;
}
...
defaultPlotOptions.radar = merge(defaultSeriesOptions, {
...
SVGElement.prototype = {
			...
			for (key in hash) {
				...
				// emulate VML's dashstyle implementation
					...
					if (value) {
						...
						i = value.length;
						while (i--) {
							value[i] = parseInt(value[i]) * hash['stroke-width'];
						}
						value = value.join(',');
					}
	...
	symbolAttr: function(hash) {
		var wrapper = this, polyRegx = /^poly\_\d+$/, sides, symbol = wrapper.symbolName;
		...
		wrapper.start = parseFloat(pick(hash.start, wrapper.start));
		wrapper.end = parseFloat(pick(hash.end, wrapper.end));
		wrapper.width = parseFloat(pick(hash.width, wrapper.width));
		wrapper.height = parseFloat(parseFloat(pick(hash.height, wrapper.height)));
		wrapper.innerR = parseFloat(pick(hash.innerR, wrapper.innerR));
                if (polyRegx.test(wrapper.symbolName)) {
                    sides = pluck(parseInt(symbol.split('_')[1], 10), 3);
                    symbol = 'poly';
                }
                else {
                    sides = undefined;
                    symbol = wrapper.symbolName;
                }

		wrapper.attr({
			d: wrapper.renderer.symbols[symbol]( wrapper.x, wrapper.y, wrapper.r, typeof sides != 'undefined' ? sides : {
				start: wrapper.start,
				end: wrapper.end,
				width: wrapper.width,
	...
	shadow: function(apply) {
		var shadows = [],
			...
			transform = this.parentInverted ? '(-1.5,-1.5)' : '(1.5,1.5)';
...
SVGRenderer.prototype = {
	...
	symbol: function(symbol, x, y, radius, options) {
                var obj, polyRegx = /^poly\_\d+$/, symbolFn, path,
                imageRegex = /^url\((.*?)\)$/, imageSrc, oriSymbol = symbol;

                if (polyRegx.test(symbol)) {
                    options = symbol.split('_')[1];
                    symbol = 'poly';
                }
	 ...
	 */
	symbols: {
                'poly' : function (x, y, radius, sides) {
                    var algle = 2 * math.PI / sides, path, j, startAngle = math.PI / 2;
                    /////initialize point attr
                    //moove to the first point
                    if (sides > 2) {
                        path = [M, (x + (radius * mathCos(-startAngle))), (y + (radius * mathSin(-startAngle))), L];


                        //loop fpr the sides
                        for (j = 1; j < sides; j++) {
                            path.push((x + (radius * mathCos(-(startAngle + (j * algle))))));
                            path.push((y + (radius * mathSin(-(startAngle + (j * algle))))));
                        }
                        path.push('Z');
                    }
                    else {
	...
	shadow: function(apply) {
		...
		if (apply) {
			for (i = 1; i <= 3; i++) {
				...
				// apply the opacity
				markup = ['<stroke color="black" opacity="', (0.1 * i), '"/>'];
	...
	function Axis (chart, options) {
		 ...
		 */
                ...
                if (min === UNDEFINED) { // points in a single hidden series, axis has no min or max
                    ...
                    if (isLogAxis) {
                        tMin = getLog(min, base);
                        returnValue = sign * (getLog(val, base) - tMin) * (axisLength / (getLog(max, base) - tMin)) + cvsOffset; //[LOG] from value to chart pixel
                    }
                    else {
		 ...
		 */
		function drawPlotLine(value, color, width, dashstyle, value2) {
			...
			if (width) {
				...
				if (horiz) {
					...
					y2 = chartHeight - marginBottom;
                                        if (value2 || value2 === 0) {
                                            x2 = transB + translate(value2);
                                            if (x2 < plotLeft) {
                                                y2 = y1 + (plotLeft - x1) / (x2 - x1) * (y2 - y1);
                                                x2 = plotLeft;
                                            }
                                            else if (x2 > plotLeft + plotWidth) {
                                                y2 = y1 + (plotLeft + plotWidth - x1) / (x2 - x1) * (y2 - y1);
					...
					x2 = chartWidth - marginRight;
                                        if (value2 || value2 === 0) {
                                            y2 = y1 + translatedValue - translate(value2);
                                            if (y2 < plotTop) {
                                                x2 = x1 + (plotTop - y1) / (y2 - y1) * (x2 - x1);
                                                y2 = plotTop;
                                            }
                                            else if (y2 > plotTop + plotHeight) {
                                                x2 = x1 + (plotTop + plotHeight - y1) / (y2 - y1) * (x2 - x1);
		...
		function drawPlotBand(from, to, color) {
			...
			var width = mathAbs(to - from) * transA;
		 ...
		 */
		function addTick(pos, tickPos, color, width, len, withLabel, index, configuration) {
                        if (!configuration) {
                            configuration = {};
                        }
			var x1, y1, x2, y2, str, labelOptions = options.labels, 
                            validLabel = false,
                            labelStyle = labelOptions.style,
                            labelRotation = labelOptions.rotation,
                            labelAlign = labelOptions.align,
                            labelOnly = configuration.labelOnly,
                            valueOnRight = configuration.valueOnRight;
			// negate the length
			if (tickPos == 'inside') {
				len = -len;
			}
			...
			// write the label
			if (labelOnly || withLabel && labelOptions.enabled) {
				str = labelOnly && typeof index === 'string' ? index :
                                    labelFormatter.call({
					index: index,
				...
				if (str || str === 0) {
                                    ...
                                    y1 = y1 + labelOptions.y - (tickmarkOffset && !horiz ?
                                            tickmarkOffset * transA * (reversed ? 1 : -1) : 0);
                                    // for plotband-like trendzone draw label on
                                    // center.

                                    if (labelOnly) {
                                        // convert rgba to hash
                                        labelStyle = jQuery.extend(true, {}, labelOptions.style, {
                                            color: color.replace(/rgba\((.*),.*\)$/, 'rgb($1)')
                                        });

                                        if (horiz) {
                                            y1 = y1 + (labelOptions.enabled ?
                                                parseFloat(labelOptions.style.fontSize) + 12 : 1);
                                            labelRotation = undefined;
                                            labelAlign = 'center';

                                            if (width > 0) {
                                                x1 = (x1 + translate(width + tickmarkOffset) + transB) / 2;
                                            }
                                        }
                                        if (!horiz && width > 0 && typeof index === 'string') {
                                            y1 = (y1 + chartHeight - translate(width + tickmarkOffset) - transB) / 2;
                                        }
...
///if clause added and rest as else to supply divlines positions previously
                        if (options.divLines instanceof Array) {
                            tickPositions = options.divLines;

                        } else {
                            tickPositions = [];

                            //for xAxis add only category positions
                            if (xOrY == 'x' && categories) {
                                if (categories instanceof Array) {
                                    ...
                                    for (i in categories) {
                                        if (categories[i] && i !== 'length'){
                                            tickPositions.push(parseInt(i));
                                        }
                                    }
                                }
                            }
		 ...
		 */
                ...
                //Changed For FC:AP
                ///added isMax argument to get the max pixel of the axis
		function getThreshold(threshold, isMax) {
                if (isMax) {
		...
		function render() {
			var axisTitleOptions = options.title,
				...
				hasData = associatedSeries.length && defined(min) && defined(max),
                                lastLabelPos, fontsize = parseInt(options.labels.style.fontSize) || 10,
                                lIndex = 0;
			...
			if (hasData || isLinked) {
				...
				if (alternateGridColor) {
					each(tickPositions, function(pos, i) {
                                            ///changed for grid line same as FUSIONCHARTS  === to !==
						if (i % 2 !== 0 && pos < max) {
							drawPlotBand(
								pos,
				...
				each (plotBands, function(plotBand) {
					drawPlotBand(plotBand.from, plotBand.to, plotBand.color, plotBand.FCconf);
                                        if (plotBand.showLabel) {
                                            addTick(plotBand.from, options.tickPosition,
                                                plotBand.color, plotBand.to, 0,
                                                true, pluck(plotBand.label, 0), {
                                                    labelOnly: true
                                                });
                                        }
				});
					...
					for (var i = min; i <= max; i += minorTickInterval) {
						...
						if (minorTickWidth) {
							addTick(
                                ...
                                //if it isi not an x axis then draw the min
                                //for x axix it will be drawn during drawing divlines at the next block
                                if (xOrY != 'x') {
                                    ///add max lable
                                    addTick(
                                        min,
					...
					tickmarkPos = pos + tickmarkOffset;
                                        ...
                                        if (xOrY == 'x' || (pos != min && pos != max)) {
                                            ...
                                            drawPlotLine(
                                                ...
                                                if (typeof lastLabelPos == 'undefined' || (mathAbs(translate(pos) - lastLabelPos) > (fontsize + 4) && (xOrY == 'x' || mathAbs(translate(pos) - translate(max)) > (fontsize + 4)))) {
                                                       ...
                                                       if ( addTick(
                                                                ...
                                                                options.tickWidth,
                                                                options.tickLength,
                                                                !(options.showdivlinevalues == false),
                                                                index
                                                        ) ) {
                                                           lastLabelPos = translate(pos);
                                                        }
                                                }
                                            lIndex = index;
                                        }
                                ...
                                //if it is not an x axis then draw the max
                                //for x axix it is drawn alraady during drawing divlines at the previous block
                                if (xOrY != 'x') {
                                    ///add max lable
                                    addTick(
                                        max,
				...
				each (plotLines, function(plotLine) {
					drawPlotLine(plotLine.value, plotLine.color, plotLine.width, plotLine.dashstyle, plotLine.to);
                                        if (plotLine.showLabel) {
                                            addTick(plotLine.value, options.tickPosition,
                                                plotLine.color, 0, 0, true,
                                                pluck(plotLine.label, 0), {
                                                    labelOnly: true
                                                });
                                        }
				});
		...
		function renderItem(item) {
			...
			if (!li) { // generate it once, later move it
				...
				// draw the marker
					...
					.attr(extend(extend(item.pointAttr[NORMAL_STATE]), {r: symbolWidth / 2}))
					.attr({zIndex: 3})
                                        .on('click', onClick)
                                        .css({'cursor': 'pointer'})
					.add(legendGroup);
	...
	function getContainer() {
		...
		chart.chartWidth = chartWidth = pick(optionsChart.width, (renderToClone || renderTo).offsetWidth, 400);
		chart.chartHeight = chartHeight = pick(optionsChart.height,
			// the offsetHeight of an empty container is 0 in standard browsers, but 19 in IE7:
                    ///fixedd FC height issue as it is alwase set in the container
			renderToOffsetHeight, 300);
		...
		if (labels.items) {
			each (labels.items, function () {
				var style = extend (labels.style, this.style),
					...
					y = parseInt(style.top, 10) + plotTop,
                                        rotation = labels.rotation || 0,
                                        align = this.align || 'left',
                                        fontsize = parseInt(style.fontSize, 10) || 12,
                                        vAlign = this.vAlign || 'top';
                                        if (vAlign.toLowerCase() == 'bottom') {
                                            y += fontsize
	...
	chart.FCAxesCons = Axis;//changed for FC by AP for candlestick
...
Point.prototype = {
	...
	onMouseOver: function() {
		...
		// hover this
                //Fixed Bug #620  comment next line
...
function doPostAnimationRoutines (series) {
...
function doPreAnimationRoutines (series) {
    if (!(series && series.options)) {
        return;
    }
};
...
Series.prototype = {
	...
	getSegments: function() {
		var lastNull = -1,
			segments = [],
			data = this.data;
                if (this.chart.options.chart.FCconf.connectnulldata == '1') {
                    // create the segments
		...
		while (i--) {
			...
			if (yValue !== null) {
				point.plotY = yAxis.translate(yValue, 0, 1);
                        ...
                        //
                    /*    if (yAxis.options.PCreversed === true) {
                               point.plotY = yAxis.getThreshold(0,true) - point.plotY;
                        }*/
		...
		if (init) { // initialize the animation
			...
			clipRect.animate({
				...
				complete: function() {
                                    ...
                                    doPostAnimationRoutines(series);
				},
				duration: 1000
			});
	...
	drawPoints: function(){
		var series = this,
                    ...
                    radius,
                    graphic,
                    barWhf, errorH, midX, midY,
                    sFCconf = series.options.FCconf || {},
                    cFCconf = chart.options.chart.FCconf, halfHEBar, halfVEBar, hColor, vColor,
                    vBarWidth, hBarWidth, vBarThickness, hBarThickness, vError, hError,
                    vErrorValue, hErrorValue;

                    vBarThickness = pluck(sFCconf.verticalerrorbarthickness,
                        sFCconf.errorbarthickness,
                        cFCconf.verticalerrorbarthickness,
                        cFCconf.errorbarthickness, 1);

                    vBarWidth = pluck(sFCconf.verticalerrorbarwidth,
                        sFCconf.errorbarwidth,
                        cFCconf.verticalerrorbarwidth,
                        cFCconf.errorbarwidth, 5);
                    ...
                    vColor = convertColor(pluck(sFCconf.verticalerrorbarcolor,
                        sFCconf.errorbarcolor, cFCconf.verticalerrorbarcolor,
                        cFCconf.errorbarcolor, 'AAAAAA'),
                        pluck(sFCconf.verticalerrorbaralpha,
                        sFCconf.errorbaralpha,
                        cFCconf.verticalerrorbaralpha,
                        cFCconf.errorbaralpha, 100));

                    if (series.options.FCtype == 7) {///7 means it is an scatter series

                        hBarThickness = pluck(sFCconf.horizontalerrorbarthickness,
                            sFCconf.errorbarthickness,
                            cFCconf.horizontalerrorbarthickness,
                            cFCconf.errorbarthickness, 1);

                        hBarWidth = pluck(sFCconf.horizontalerrorbarwidth,
                            sFCconf.errorbarwidth,
                            cFCconf.horizontalerrorbarwidth,
                            cFCconf.errorbarwidth, 5);
                        ...
                        hColor = convertColor(pluck(sFCconf.horizontalerrorbarcolor,
                            sFCconf.errorbarcolor, cFCconf.horizontalerrorbarcolor,
                            cFCconf.errorbarcolor, 'AAAAAA'),
                            pluck(sFCconf.horizontalerrorbaralpha,
                            sFCconf.errorbaralpha,
                            cFCconf.horizontalerrorbaralpha,
                            cFCconf.errorbaralpha, 100));
                    }

                    // do not let the bar thickness exceed line-width
                    if (series.options.lineWidth) {
                        vBarThickness = vBarThickness > series.options.lineWidth ?
                            series.options.lineWidth : vBarThickness;
                        hBarThickness = hBarThickness > series.options.lineWidth ?
                            series.options.lineWidth : hBarThickness;
                    }
					...
					if (graphic) { // update
						graphic.attr({
							...
							r: radius
                                                ...
                                                if (series.options.FCtype == 7) {
                                                    vError = pluck(pFCconf.useverticalerrorbar, sFCconf.useverticalerrorbar, cFCconf.useverticalerrorbar, 1);
                                                    vErrorValue =  parseFloat(pick(pFCconf.verticalerrorvalue, pFCconf.errorvalue)) || 0;
                                                    hError = pluck(pFCconf.usehorizontalerrorbar, sFCconf.usehorizontalerrorbar, cFCconf.usehorizontalerrorbar, 0);
                                                    hErrorValue =  parseFloat(pick(pFCconf.horizontalerrorvalue, pFCconf.errorvalue)) || 0;
                                                }
                                                else {
                                                    vError = 1;
                                                    vErrorValue = parseFloat(pFCconf.errorvalue) || 0;
                                                    hError = 0;
                                                    hErrorValue = 0;
                                                }

                                                ///if need draw the h error bar
                                                if (hError == '1' && hErrorValue != 0) {
                                                    barWhf = hBarWidth / 2;
                                                    point.error = {
                                                        shapeType: 'path',
                                                        pointArgs: {
                                                            'stroke-opacity': "1",
                                                            'stroke': hColor,
                                                            'stroke-width': hBarThickness
                                                        }
                                                    };
                                                    errorH = mathAbs(series.xAxis.getThreshold(series.xAxis.options.max - mathAbs(hErrorValue)) - series.xAxis.getThreshold(series.xAxis.options.max, true));
                                                    ...
                                                    ///draw the upper half
                                                    point.error.shapeArgs = [M, midX, midY, L, (midX + errorH), midY,
                                                        (midX + errorH), (midY - barWhf),
                                                        (midX + errorH), (midY + barWhf),
                                                        (midX + errorH), midY, 'Z'];
                                           
                                                    ///draw the error
                                                    point.error.hGraphic = series.chart.renderer[point.error.shapeType](point.error.shapeArgs)
                                                            .attr(point.error.pointArgs)
                                                            .on('mouseover', function(point, chart) {
                                                                return function() {
                                                                    //chart.
                                                                ...
                                                                series : series,
                                                                tooltipText : hErrorValue,
                                                                tooltipPos : [midX + (errorH / 2) + 25, plotY]
                                                            }, chart))
                                                            .add(series.group);
                                                    ...
                                                    if (halfHEBar != 1) {
                                                        ...
                                                        ///draw the error
                                                        point.error.hGraphic1 = series.chart.renderer[point.error.shapeType](point.error.shapeArgs)
                                                                .attr(point.error.pointArgs)
                                                                .on('mouseover', function(point, chart) {
                                                                    return function() {
                                                                        //chart.
                                                ...
                                                ///if need draw the v error bar
                                                if (vError == '1' && vErrorValue != 0) {
                                                    barWhf = vBarWidth / 2;
                                                    ...
                                                    point.error = {
                                                        ...
                                                        pointArgs: {
                                                           // 'stroke-opacity': "1",
                                                            'stroke': vColor,
                                                            'stroke-width': vBarThickness
                                                        }
                                                    };

                                                    errorH = mathAbs(series.yAxis.getThreshold(series.yAxis.options.max - mathAbs(vErrorValue)) - series.yAxis.getThreshold(series.yAxis.options.max, true));
                                                    ...
                                                    ///draw the upper half
                                                    point.error.shapeArgs = [M, midX, midY, L, midX, (midY - errorH),
                                                        M, (midX - barWhf), (midY - errorH), L,
                                                        (midX + barWhf), (midY - errorH)];

                                                    ///draw the error
                                                    point.error.vGraphic = series.chart.renderer[point.error.shapeType](point.error.shapeArgs)
                                                            .attr(point.error.pointArgs)
                                                            .on('mouseover', function(point, chart) {
                                                                return function() {
                                                                    //chart.
                                                                ...
                                                                series : series,
                                                                tooltipText : vErrorValue,
                                                                tooltipPos : [plotX, midY - (errorH / 2) - 10]
                                                            }, chart))
                                                            .add(series.group);
		...
		if (this.options.dataLabels.enabled) {
			var series = this,
				...
				seriesType = series.type,
                                valuePosition = (series.options.FCconf && series.options.FCconf.valueposition) || (series.chart.options.chart.FCconf && series.chart.options.chart.FCconf.valueposition),
                                color,
				align;
			...
			each(data, function(point){
				...
				var plotX = pick(point.barX, point.plotX),
					...
					pointLabel = point.dataLabel,
                                        positionY,
                                        pointvaluePosition = (point.options.FCconf && point.options.FCconf.valueposition) || valuePosition || 'auto';
                                        
				// destroy old data label after update
				if (pointLabel) {
					point.dataLabel = pointLabel.destroy();
				}
                                positionY = ((options.style && parseInt(options.style.fontSize)) || 6) + 4;
                                ////get the y position for the label depending upon the value position
                                if (pointvaluePosition.toLowerCase() === 'above' || (pointvaluePosition.toLowerCase() === 'auto' && plotY > positionY)) {
				...
				y = (inverted ? chart.plotHeight - plotX : plotY) + positionY;

				// special case for pies
				if (tooltipPos) {
					x = tooltipPos[0] + options.x;
					y = tooltipPos[1] + positionY;
				}
	...
	drawGraph: function(state) {
		var series = this,
			...
			renderer = chart.renderer,
                        //Changed For FC:AP
                        //get the max thresold if it is an inversed chart
                        translatedThreshold = series.yAxis.getThreshold(options.threshold || 0, !series.chart.options.FCconf.negative && series.yAxis.options.PCreversed),
				...
				each(segment, function(point, i){
					...
					if (i == 0) {
						segmentPath.push(M);
                                    ...
                                    //changed for stepcharts
                                    if (options.useforwardsteps !== false) {
					...
					if (i && options.step) {
						...
						segmentPath.push (
							...
							lastPoint.plotY
                                        ...
                                        ////changed for FC
                                        //step line chart
                                        if (i && options.drawverticaljoins === false) {
                                            segmentPath.push(M)
                                        }
					...
					segmentPath.push(
						...
						point.plotY
                                        ...
                                        ///changed for FC
                                        //step line chart
                                        if ( i && i < segment.length - 1 && options.drawverticaljoins === false) {
                                            segmentPath.push(L)
                                        }
                                    }
                                    else {
                                        // moveTo or lineTo
					if (i == 1 && options.drawverticaljoins !== false) {
						segmentPath.push(L);
					}
					// step line?
					if (i && options.step) {
		...
		if (useArea) {
			fillColor = pick(
				options.fillColor,
				Color(series.color).setOpacity(options.fillOpacity || 0.70).get()
			);
			if (area) {
...
var ColumnSeries = extendClass(Series, {
	...
	drawDataLabels: function() {
		if (this.options.dataLabels.enabled) {
			var series = this,
				...
				inverted = chart.inverted,
				seriesType = series.type,
                                valuePosition = (series.chart.options.chart.FCconf && series.chart.options.chart.FCconf.placevaluesinside == '1') ? true : false,
                                color,
				align;
			...
			each(data, function(point){
                            if (point.y != null){
                                var plotX = pick(point.barX, point.plotX),
                                    ...
                                    if (inverted){//bar chart
                                        x = chart.plotWidth - plotY;
                                        y = chart.plotHeight - plotX;
                                        if (valuePosition || (chart.plotWidth - x) < 50) {
                                            align = 'right';
                                        }
                                    }
                                    else{//column chart
                                        fontHeight = (options.style && parseInt(options.style.fontSize)) || 6;
                                        ////get the y position for the label depending upon the value position

                                        if ((valuePosition && point.y > 0) || plotY < (fontHeight + 2)) {
                                            y = plotY + fontHeight + 2;
                                        }
                                        else {
                                            y = plotY - 2;
                                        }
	...
	translate: function() {
		var series = this,
			...
			categories = series.xAxis.categories,
                        sFCconf = series.options.FCconf || {},
                        cFCconf = chart.options.chart.FCconf,
                        errorColor = convertColor(pluck(sFCconf.errorbarcolor,
                            cFCconf.errorbarcolor, 'AAAAAA'),
                            pluck(sFCconf.errorbaralpha,
                            cFCconf.errorbaralpha, 100)),
                        errorThickness = pluck(sFCconf.errorbarthickness,
                            cFCconf.errorbarthickness, 1),
                        barWidthPercent = parseInt(pluck(sFCconf.errorbarwidthpercent, cFCconf.errorbarwidthpercent, 70), 10),
		...
		each (data, function(point) {
                    var barX = pick(point.MX, point.plotX + pointXOffset),
                                barH = mathCeil(mathAbs((point.yBottom || translatedThreshold) - point.plotY)),
				plotY = point.plotY,
				barY = mathCeil(point.MY ? series.yAxis.getThreshold(point.MY) : mathMin(plotY, translatedThreshold)),
				barW = pick(point.MWidth, pointWidth),
				trackerY, barWhf, errorH, midX, midY;
			...
			if (mathAbs(barH) < (minPointLength || 5)) {
				...
				trackerY = barY - 3;
                        ...
                        //Changed For FC:AP
                        //if the chart is an inversed chart then change the chart's point args
                        //check whether the chart is an reversed chart
                        //getThreshold is changed to get the max pixel call getThreshold(0,true)
                        if (!series.chart.options.FCconf.negative && point.series.yAxis.options.PCreversed === true) {
                                barY = barH;
                                barH = point.series.yAxis.getThreshold(0,true) - barY;
                        }
			...
			point.shapeArgs = {
				...
				r: options.borderRadius
                        ...
                        //add the error shape
                        if (point.errorvalue) {
                            barWhf = barW * barWidthPercent / 200;

                            if (isNaN(barWhf)) {
                                barWhf = barW * 70 / 200;
                            }
                            errorH = mathAbs(series.yAxis.getThreshold(series.yAxis.options.max - mathAbs(point.errorvalue)) - series.yAxis.getThreshold(series.yAxis.options.max, true));
                            if (point.errorvalue < 0) {
                                errorH = -errorH;
                            }
                            midX = barX + (barW / 2);
                            midY = barY + (point.y < 0 ? barH : 0);
                            point.error = {
                                shapeType: 'path',
                                errorH : errorH,
                                midX : midX,
                            ...
                            point.error.shapeArgs = [M, midX, midY, L, midX, (midY - errorH),
                                ...
                                (midX + barWhf), (midY - errorH)];
                            ///draw the lower half
                            if (series.chart.options.chart.halferrorbar != 1) {
                                point.error.shapeArgs1 = [M, midX, midY, L, midX, (midY + errorH),
                                    M, (midX + barWhf), (midY + errorH), L,
                                    (midX - barWhf), (midY + errorH)];
                            }
				...
				if (graphic) { // update
					...
					point.graphic = renderer[point.shapeType](shapeArgs)
						...
						.shadow(options.shadow);
                                                ...
                                                //draw the error bar
                                                if (point.error) {
                                                    point.error.graphic = renderer[point.error.shapeType](point.error.shapeArgs)
                                                            .attr(point.error.pointArgs)
                                                            .on('mouseover', function(point, chart) {
                                                                    ...
                                                                    series : series,
                                                                    tooltipText : point.errorvalue,
                                                                    tooltipPos : [point.error.midX, point.error.midY - (point.error.errorH / 2) - 10]
                                                                }, chart))
                                                            .add(series.group)
                                                            .shadow(false);
                                                    if (point.error.shapeArgs1) {
                                                        point.error.graphic = renderer[point.error.shapeType](point.error.shapeArgs1)
                                                        .attr(point.error.pointArgs)
                                                        .on('mouseover', function(point, chart) {
	...
	drawTracker: function() {
		...
		each (series.data, function(point) {
			...
			shapeType = point.trackerShapeType || point.shapeType;
...
var CandleStickSeries = extendClass(ColumnSeries, {
	type: 'candlestick',
...
/**
	...
	translate: function() {
		var series = this,
			...
			stackedIndex, // the index of the first column in a stack
                        //renderer = chart.renderer,
                        volumeChart = chart.options.FCconf.volumeChart || {},
                        optionsChart = jQuery.extend(true, {}, volumeChart), vContainer,
                        pricetype = series.options.priceType;
                        Series.prototype.translate.apply(series);
			    /////////draw volume charts
                if (optionsChart.shoVolumn) {
                   vContainer = createElement('div');
                   vContainer.style.width = optionsChart.chart.width + 'px';
                   vContainer.style.height = optionsChart.chart.height + 'px';
                   vContainer.style.margin = '-' + optionsChart.FCconf.marginTop + 'px auto 0 auto';
                   vContainer.style.background = 'transparent';
                   
                   chart.container.appendChild(vContainer);
                   optionsChart.chart.renderTo = vContainer;
                   new Highcharts.Chart(optionsChart);                    
                   optionsChart.shoVolumn = false;
...
///column3d series
var Column3dSeries = extendClass(ColumnSeries, {
	type: 'column3d',
	...
	drawPoints: function() {
		var series = this,
			...
			graphic,
			shapeArgs,
                        ////this will be fexible so that we canchange the viewing angle
                        xChange = 10,
                        yChange = -12;
				...
				if (graphic) { // update
					...
					.add(series.group);
						...
						.shadow(options.shadow);
                                                ...
                                                ///addd the upper path
                                                renderer['path']([M, shapeArgs.x, shapeArgs.y, L, (shapeArgs.x + shapeArgs.width),
                                                shapeArgs.y, (shapeArgs.x + shapeArgs.width + xChange), (shapeArgs.y + yChange),
                                                (shapeArgs.x + xChange), (shapeArgs.y + yChange), 'Z'])
						.attr(point.pointAttr[point.selected ? SELECT_STATE : NORMAL_STATE])
						.add(point.graphic);

                                                ///add the side
                                                renderer['path']([M, (shapeArgs.x + shapeArgs.width), shapeArgs.y,  L,
                                                    (shapeArgs.x + shapeArgs.width + xChange), (shapeArgs.y + yChange), (shapeArgs.x + shapeArgs.width + xChange), (shapeArgs.y + shapeArgs.height + yChange),
                                                    (shapeArgs.x + shapeArgs.width), (shapeArgs.y + shapeArgs.height), 'Z'])
						.attr(point.pointAttr[point.selected ? SELECT_STATE : NORMAL_STATE])
						.add(point.graphic);
                                                //Changed for FC
                                                //draw the error bar
...
seriesTypes.column3d = Column3dSeries;
 ...
 *dragnode chart
 */
function determineShape (DragNodeConf) {
   switch (DragNodeConf.shape && DragNodeConf.shape.toLowerCase()) {
        case 'polygon':
           return 'polygon' ;
            break;
        case 'circle':
...
var DragNode = extendClass(ScatterSeries, {
    type: 'dragnode',
    /**
	 * Extend the base Series' translate method by adding shape type and
	 * arguments for the point trackers
	...
	translate: function() {
		var series = this;
                ///create the node container for the first time
                if (!series.chart.nodeContainer) {
                    series.chart.nodeContainer = {};
                }
                var nodeContainer = series.chart.nodeContainer;

		Series.prototype.translate.apply(series);

		each (series.data, function(point) {
                    var conf = point.FCconf || {};
                        //save the node as the id as index
                        if (conf.id){
                           nodeContainer[conf.id] = point;
                        }

                        //add display value for drag node
                        point.options.displayvalue = defined(conf.name) ? conf.name :(defined(conf.label) ? conf.label : '');
                        point.options.tooltext = pluck(conf.tooltext, conf.hovertext, conf.name);
        ...
        /**
	 * Draw the data labels
         * THis is modify to draw all labels tag only
         *  as all labels for the series already drawn during the point darw
	 */
	drawDataLabels: function() {
		var series = this,
                chart = series.chart,
                labels = chart.options.FClabels, X, Y, text, color, alpha, bgcolor,
                labelBox, cLabelH, cLabelW,
                bordercolor, fontSize, padding, labelGrapic, label, style, xAxis = series.xAxis, yAxis = series.yAxis;
                if (!chart.dragLabels && labels) {
                   
                    //create the group
                    chart.dragLabels = chart.renderer.g(PREFIX +'drag-labels')
                    .attr({
                            visibility: VISIBLE,
                            zIndex: 4
                    })
                    ...
                    for (var i = 0; i < labels.length; i += 1) {
                        label = labels[i];
                        text = label.text;
                        X = xAxis.translate(label.x);
                        Y = yAxis.translate(label.y, 0, 1);
                        style = jQuery.extend(true, {}, chart.options.tooltip.style);
                        style.color = label.color ? label.color.replace(/\#*(.)/, '#$1') : style.color;
                        style.fill = style.color;
                       //// Has no major effect alpha = pluck(label.alpha, 100);
                        style.background = label.bgcolor ?  label.bgcolor.replace(/\#*(.)/, '\#$1') : undefined;
                        bordercolor = label.bordercolor.replace(/\#*(.)/, '\#$1');
                        style.border = label.bordercolor ?  '1px solid ' + bordercolor : undefined;
                        style.fontSize = label.fontsize ? parseInt(label.fontsize, 10) + 'px' : style.fontSize;
                        padding = label.padding ? parseInt(label.padding, 10) : 5;
                        style.padding = padding + 'px';


                       labelGrapic = chart.renderer.text(
                                text,
                                chart.plotLeft + X,
                                chart.plotTop + Y + (parseInt(style.fontSize, 10) / 2),
                                style,
                                false,
                                'center'
                        ).attr({
                        ...
                        .add(chart.dragLabels);
                         //draw the label box //for SVG only
                        if (typeof labelGrapic.element.getBoundingClientRect == 'function'){
                            labelBox = labelGrapic.element.getBoundingClientRect();
                            cLabelH = parseInt(style.fontSize, 10) + (2 * padding);
                            cLabelW = (labelBox.width || labelGrapic.element.getComputedTextLength() || 1) + (2 * padding);
                            labelGrapic.labelRect = chart.renderer['rect']({
                                        x: chart.plotLeft + (X - (cLabelW / 2)),
                                        y: chart.plotTop + (Y - (cLabelH / 2)),
                                        width: cLabelW,
                                        height: cLabelH
                            }).attr({
                                        zIndex: 1,
                                        fill: Color(style.background).get('rgb'),
                                        stroke: Color(bordercolor).get('rgb')
                                });
                                chart.dragLabels.element.insertBefore(labelGrapic.labelRect.element, labelGrapic.element);
                        }
                    }
                }
	...
	drawPoints: function(){
		var series = this,
			...
			plotX,
			plotY,
                        pointLabel, conectColor, positionY, str, labeyY,
			i, j, k,
			point, rectH, rectW,
			radius, strokeHWidth,
                        dataLabelsGroup = series.dataLabelsGroup,
			labelHeight = parseInt(series.options.dataLabels.style.fontSize, 10),
                        cLabelW, cLabelH, labelBox, DragNodeConf, sides,
                        startAngle = math.PI / 2, markeAttr,
                        algle, width, height, id, connector, link, conectArr,
                        cFCconf = chart.options.chart.FCconf,
                        sFCconf = series.options.FCconf,
                        connectors,
                        options = series.options.dataLabels, color, shape;
                        
                if (isNaN(labelHeight)) {
                    labelHeight = 10;
                }
                if (!chart.connectors && chart.options.connectors instanceof Array) {
                    chart.connectors = jQuery.extend(true, [], chart.options.connectors)
                }
		...
		if (series.options.marker.enabled) {
			...
			for (i = 0; i < data.length; i += 1) {
				...
				plotY = point.plotY;
                                pointLabel = point.dataLabel,
                                DragNodeConf = point.options.FCconf;
                                shape = determineShape(DragNodeConf);
                                radius = pluck(parseFloat(DragNodeConf.radius), 10);// default radius  as 2 px
                                sides = pluck(parseInt(DragNodeConf.numsides, 10), 4);//default 4
				width = pluck(parseFloat(DragNodeConf.width), 10);
                                height = pluck(parseFloat(DragNodeConf.height), 10);
                                // only draw the point if y is defined
				if (plotY !== UNDEFINED) {
                                        // destroy old data label after update
                                        if (pointLabel) {
					...
					// shortcuts
                                        id = DragNodeConf.id;
                                        //if the node has an id then check for the connectors
                                        if (id && connectors instanceof Array) {
                                            
                                            if (!chart.connectorsGroup) {
                                                chart.connectorsGroup = chart.renderer.g('axis');
                                                chart.connectorsGroup.attr({
                                                    zIndex: 2,
                                                    width: chart.plotWidth,
                                                    height: chart.plotHeight
                                                })
                                            ...
                                            for (j = 0; j < connectors.length; j += 1) {
                                                    ...
                                                    if (typeof connectors[j] === 'object' && connectors[j].connector) {
                                                        for (k = 0; k < connectors[j].connector.length; k += 1) {
                                                            connector = connectors[j].connector[k];
                                                            conectColor = convertColor(connector.color || connectors[j].color, connector.alpha || connectors[j].alpha);
                                                            if ((connector.from == id && connector.donefrom != true) || (connector.to == id && connector.doneto != true)) {///check link this point
                                                                link = connector.from == id ? 'from' : 'to';
                                                                
                                                                connector['done' + link] = true; ///solved bug 772 morethen 1 node with same id
                                                                 if (connector['_SHAPE']) {
                                                                    conectArr = [M, plotX, plotY, L];
                                                                    ///draw the arrow if required
                                                                    if (connector.arrowatstart != '0') {
                                                                        if (link == 'from') {
                                                                            drawArrow(conectArr, plotX, plotY, connector['_SHAPE'].x, connector['_SHAPE'].y, (shape == 'rectangle' ? width : radius) + connector['_SHAPE'].strokeWidth, shape == 'rectangle'? height + connector['_SHAPE'].strokeWidth: undefined);
                                                                        }
                                                                        else {                                                                        
                                                                            drawArrow(conectArr, connector['_SHAPE'].x, connector['_SHAPE'].y, plotX, plotY, connector['_SHAPE'].r, connector['_SHAPE'].H);
                                                                        }

                                                                    }
                                                                    if (connector.arrowatend != '0') {
                                                                        if (link == 'to') {
                                                                            drawArrow(conectArr, plotX, plotY, connector['_SHAPE'].x, connector['_SHAPE'].y, connector['_SHAPE'].strokeWidth + (shape == 'rectangle' ? width : radius), shape == 'rectangle'? height + connector['_SHAPE'].strokeWidth : undefined);
                                                                        }
                                                                        else {
                                                                           drawArrow(conectArr, connector['_SHAPE'].x, connector['_SHAPE'].y, plotX, plotY, connector['_SHAPE'].r, connector['_SHAPE'].H);
                                                                         }
                                                                    ...
                                                                    ///draw the label
                                                                    if (connector.label || connector.label == 0) {
                                                                        connector.labelGrapic = chart.renderer.text(
                                                                                connector.label,
                                                                                (plotX + connector['_SHAPE'].x) / 2,
                                                                                (((plotY + connector['_SHAPE'].y) / 2) + (labelHeight / 2)),
                                                                                {
                                                                                    fontSize: options.style.fontSize,
                                                                                    color: options.style.color,
                                                                                    fontFamily: options.style.fontFamily,
                                                                                    background: Color(conectColor).get('rgb')
                                                                                },
                                                                                false,
                                                                                'center'
                                                                        ).attr({
                                                                        ...
                                                                        .add(chart.connectorsGroup);
                                                                        //draw the label box //for SVG only
                                                                        if (typeof connector.labelGrapic.element.getBoundingClientRect == 'function'){
                                                                            labelBox = connector.labelGrapic.element.getBoundingClientRect();
                                                                            cLabelH = 14; //fontsize is not adjustable as up now
                                                                            cLabelW = (labelBox.width || connector.labelGrapic.element.getComputedTextLength() || 1);
                                                                            connector.labelRect = chart.renderer['rect']({
                                                                                        x: ((plotX + connector['_SHAPE'].x) / 2 - (cLabelW / 2)),
                                                                                        y: ((plotY + connector['_SHAPE'].y) / 2 - (cLabelH / 2)),
                                                                                        width: cLabelW,
                                                                                        height: cLabelH
                                                                            }).attr({
                                                                                        zIndex: 1,
                                                                                        fill: Color(conectColor).get('rgb')
                                                                                });
                                                                                chart.connectorsGroup.element.insertBefore(connector.labelRect.element, connector.labelGrapic.element);
                                                                        }
                                                                    ...
                                                                    conectArr.push(connector['_SHAPE'].x);
                                                                    conectArr.push(connector['_SHAPE'].y);

                                                                    //draw the connector
                                                                    chart.renderer['path'](conectArr)
                                                                    .attr({
                                                                        zIndex: 1,
                                                                       // dashstyle: pluck(connector.dashed, connectors[j].dashed) == '1' ? pluck(connector.dashlen, connectors[j].daslen, 5) + ',' + pluck(connector.dashgap, connectors[j].dashgap, 2) : undefined,
                                                                        'stroke': conectColor,
                                                                        'stroke-width': connector['_SHAPE'].strokeWidth * 2///saved width is the half of the original
                                                                    })
                                                                    .add(chart.connectorsGroup);
                                                                    

                                                                }else {
                                                                    strokeHWidth = adjustZero(pluck(connector.strength, 1) * pluck(connectors[j].stdthickness, 1)) / 2;
                                                                    connector['_SHAPE'] = {
                                                                        x: plotX,
                                                                        y: plotY,
                                                                        r: strokeHWidth + (shape == 'rectangle' ? width : radius),
                                                                        H: shape == 'rectangle'? height + strokeHWidth : undefined,
                                                                        strokeWidth: strokeHWidth
                                                                    };
					...
					markeAttr = {
                                                        'fill': convertColor(DragNodeConf.color, pluck(DragNodeConf.alpha, sFCconf.alpha, sFCconf.plotfillalpha, sFCconf.nodefillalpha, cFCconf.plotfillalpha, cFCconf.nodefillalpha), series.color),
                                                        'stroke': convertColor(pluck(sFCconf.plotbordercolor, cFCconf.plotbordercolor, "666666"),
                                                                              pluck(sFCconf.showplotborder, cFCconf.showplotborder, 1) == 1 ? 
                                                                                  pluck(sFCconf.plotborderalpha, cFCconf.plotborderalpha, 95) : 0),
                                                        'stroke-width': pluck(sFCconf.plotborderthickness, sFCconf.nodeborderthickness, cFCconf.plotborderthickness, cFCconf.nodeborderthickness, 1)
                                        };

                                        //draw the shape
                                        //if shape type is polygon or no shape type is given but it has radious and sides
                                        if (shape == 'polygon'){///draw a polygon
                                            
                                            algle = 2 * math.PI / sides;
                                            /////initialize point attr
                                            //moove to the first point
                                            pointAttr = [M, (plotX + (radius * mathCos(-startAngle))), (plotY + (radius * mathSin(-startAngle))), L];

                                            //add width height attr
                                            //1.41 = 2 * cos(45')
                                            rectH = radius * 1.41;
                                            rectW = radius * 1.41;
                                            ...
                                            //loop fpr the sides
                                            for (j = 1; j < sides; j++) {
                                                pointAttr.push((plotX + (radius * mathCos(-(startAngle + (j * algle))))));
                                                pointAttr.push((plotY + (radius * mathSin(-(startAngle + (j * algle))))));
                                            }
                                            pointAttr.push('Z');
                                            point.graphic = chart.renderer['path'](pointAttr)
                                            .attr(markeAttr)
                                            .add(series.group);


                                        }
                                        else if (shape == 'rectangle') {//draw a rectangle
                                            //add width height attr
                                            rectH = height;
                                            rectW = width;
                                            point.graphic = chart.renderer['rect']({
                                                    x: plotX - (width / 2),
                                                    y: plotY - (height / 2),
                                                    width: width,
                                                    height: height
                                        ...
                                        else {///by default draw a circle
                                            //add width height attr
                                            rectH = radius * 1.41;
                                            rectW = radius * 1.41;

                                            markeAttr.r = undefined;
                                            point.graphic = chart.renderer.circle(
                                            plotX,
                                            plotY,
                                            radius
                                            )
                                        ...
                                        var imageUrl = pluck(DragNodeConf.imageurl);
                                        //if image node draw image
                                        if (DragNodeConf.imagenode == '1' && imageUrl) {
                                            
                                            var imageAlign = pluck(DragNodeConf.imagealign, 'top').toLowerCase(),
                                            imageWidth = pluck(DragNodeConf.imagewidth, -1),
                                            imageHeight = pluck(DragNodeConf.imageheight, -1),
                                            labelAlign = pluck(DragNodeConf.labelalign, 'top').toLowerCase(),
                                            ImgX, ImgY;               

                                            if (imageWidth > rectW || imageWidth == -1) {
                                                imageWidth = rectW;
                                            }
                                            if (imageHeight > rectH || imageHeight == -1) {
                                                imageHeight = rectH;
                                            }

                                            ImgX = plotX - (imageWidth / 2);
                                            ///if
                                            if (imageAlign == 'bottom') {
                                                ImgY = plotY + (rectH / 2) - imageHeight;
                                            }
                                            else if (imageAlign == 'middle') {
                                                ImgY = plotY - (imageHeight / 2);
                                            }
                                            else {
                                                ImgY = plotY - (rectH / 2);
                                            }
                                            ...
                                            point.ImageGraphic = chart.renderer['image'](
                                                imageUrl, ImgX, ImgY, imageWidth, imageHeight
                                                        )
                                                        .add(series.group);
                                            ...
                                            if (labelAlign == 'bottom') {
                                                labeyY = plotY + (rectH / 2);
                                            }
                                            else if (labelAlign == 'middle') {
                                                labeyY = plotY + (labelHeight / 2);
                                            }
                                            else {
                                                labeyY = plotY - (rectH / 2) + labelHeight;
                                            }



                                                        
                                            if (this.options.dataLabels.enabled && str) {
                                                    point.dataLabel = chart.renderer.text(
                                                            str,
                                                            plotX,
                                                            labeyY,
 ...
 */
var DEFAULT_PIE_SLICE_ANIMATION_DURATION = 500;
var PiePoint = extendClass(Point, {
	...
	init: function () {
		...
		addEvent(point, 'click', function () {point.clicked = true;});
	...
	slice: function(sliced, redraw) {
		...
		}, DEFAULT_PIE_SLICE_ANIMATION_DURATION);
	...
	translate: function() {
		var total = 0,
			series = this,
                        ///changed
                        // cumulative = -0.25
                        //added next 2 lines //to add pie startingangle suport
                        tempAngle = parseInt(((this.options.startingAngle % 360)/360)*100)/100,
			cumulative = tempAngle > 0.5 ? 1-tempAngle : -tempAngle , // start at right as default
			options = series.options,
		...
		each (data, function(point) {
                        if (point.y !== null) {
                            ...
                            // set the shape
                            if (data.length == 1 && point.y != 0) {
                                point.shapeType = 'circle';
                                point.slicedTranslation = [chart.plotLeft, chart.plotTop];
	...
	drawDataLabels: function() {
		if (this.options.dataLabels.enabled) {
			var series = this,
				...
				align,
                                colitionUp = 1,
                                fontSize = parseInt(options.style.fontSize, 10) || 10;
			...
			// make the labels for each point
			each(data, function(point){
				var pointLabel = point.dataLabel,
                                        shapArgs = point.shapeArgs,
                                        centerAngle;
				...
				str = options.formatter.call({
					...
					total: point.total || point.stackTotal
                                ...
                                centerAngle = (shapArgs.start + shapArgs.end) / 2;

                                if (chart.series.length > 1) {
                                    radius = (shapArgs.r + shapArgs.innerR) / 2;
                                    x1 = shapArgs.x + (radius * mathCos(centerAngle));
                                    y1 = shapArgs.y + (radius * mathSin(centerAngle));
                                    align = 'center';
                                }
                                else {
                                    dataLabelsGroup = point.group;
                                    x = shapArgs.x + (shapArgs.r * mathCos(centerAngle));
                                    y = shapArgs.y + (shapArgs.r * mathSin(centerAngle));
                                    ...
                                    if (centerAngle > math.PI && centerAngle < 2 * math.PI) {
                                        ...
                                        tempY1 = parseInt(y, 10) + 5;
                                    }
                                    if (centerAngle > (math.PI / 2) && centerAngle < 3 * (math.PI / 2)) {
                                        x1 = x - 15;
                                        //reduce the overlap condition
                                        if (mathAbs(tempY1 - y1) > fontSize ||  align != 'right') {
                                        y1 =  tempY1;
                                        }
                                        else {
                                            y1 = (parseInt(y1, 10) + fontSize);
                                        }
                                        align = 'right';
                                        ...
                                        x1 = x + 15;
                                        //reduce the overlap condition
                                        y1 = ((mathAbs(y1 - tempY1) > fontSize || align != 'left') ? tempY1 : (y1 - fontSize));
                                        align = 'left';

                                        
                                    }
                                    ////draw connectors
                                    point.connectors = chart.renderer.path(
                                        [M, x , y, L, (x1 + x) / 2, y1, x1, y1]
                                        )
					.attr({
                                                'stroke' : '#000000',//
						'stroke-width': 1 + PX,
			...
			each(data, function(point){
				...
				str = options.formatter.call({
					...
					total: point.total || point.stackTotal
                                ...
                                centerAngle = (shapeArgs.start + shapeArgs.end) / 2;

                                if (data.length == 1 && point.y != 0) {
                                    x1 = shapeArgs.x;
                                    y1 = shapeArgs.y + (fontSize / 2);
                                    align = 'center';
                                }
                                else if (chart.series.length > 1) {
                                    radius = (shapeArgs.r + shapeArgs.innerR) / 2;
                                    x1 = shapeArgs.x + (radius * mathCos(centerAngle));
                                    y1 = shapeArgs.y + (radius * mathSin(centerAngle)) + (fontSize / 2);
                                    align = 'center';
                                    
                                }
                                else {                                    
                                    x = shapeArgs.x + (shapeArgs.r * mathCos(centerAngle));
                                    y = shapeArgs.y + (shapeArgs.r * mathSin(centerAngle));
		...
		each (series.data, function(point) {
			...
			if (graphic) {
				graphic.attr(shapeArgs);
                             ...
                             if (series.data.length == 1 && point.y != 0 && shapeArgs.innerR == 0) {
                                point.graphic =
					renderer.circle(shapeArgs)
...
/**
 * The Radder series class
 */
var RadarSeries = extendClass(seriesTypes.area, {
	...
	translate: function() {
		var series = this,
			...
			i = data.length,
                        ...
                        R = mathMin((CX - 30), (CY - 20));
                        if (R < 0 ) {
                            R = mathMin(CX, CY);
                        }
                        var yMin = series.yAxis.options.min,
                        range = mathAbs(series.yAxis.options.max - yMin),
                        trans = R / range,
                        xAngle = (2 * math.PI) / categories.length,
		...
		while (i--) {
			var point = data[i],
				...
				yValue = point.y || 0;
			point.plotX = CX + ((trans * mathAbs(yValue - yMin)) * mathCos(-(startAngle + (i * xAngle))));

			point.plotY = CY + ((trans * mathAbs(yValue - yMin)) * mathSin(-(startAngle + (i * xAngle))));
			// set client related positions for mouse tracking
	...
	drawGraph: function(state) {
		var series = this,
			...
			color = series.color,
			lineWidth = options.lineWidth,
			lineColor = options.lineColor || series.color,
			fillCOlor = pick(
				options.fillColor,
				Color(series.color).setOpacity(options.fillOpacity || 0.5).get()
			),
			segmentPath,
			renderer = chart.renderer,
                        singlePoints = []; // used in drawTracker
...
/**
	...
	drawTracker: function() {
        ...
        plotRadarAxis: function () {
            var series = this,
			chart = series.chart,
                        xCat = series.xAxis.categories,
			categoriesLN = xCat.length,
			renderer = series.chart.renderer,
			i,
                        CX = chart.plotWidth / 2,
                        CY = chart.plotHeight / 2,
                        R = mathMin((CX - 30), (CY - 20));
                        //if radious become negative
                        if (R < 0) {
                            R = mathMin(CX, CY);
                        }
                        var yMax = series.yAxis.options.max,
                        yMin = series.yAxis.options.min,
                        range = mathAbs(yMax - yMin),
                        xAngle = (2 * math.PI) / categoriesLN,
                        startAngle = math.PI / 2,
                        axPath = [],
                        brPath = [M],
                        numdivPath = [],
                        DivGap = series.yAxis.options.tickInterval,
                        numDiv = mathCeil(range / DivGap) - 1,
                        trans = R / range,
                        first,
                        j, Px, Py, length,
                        str,
                        x1, y1, value, angle,
                        labelOptions = series.yAxis.options.labels,
                        labelFormatterY = series.yAxis.options.labels.formatter,
                        labelOptionsX = series.xAxis.options.labels,
                        labelFormatterX = series.xAxis.options.labels.formatter,
                        FCconf = chart.options.chart.FCconf,
                ...
                ////create the min value
                //draw the yAxis min labels
                // 
                if (FCconf.showyaxisvalues != 0 && FCconf.showlimits != '0') {
                    value = yMin;
                        str = labelFormatterY.call({
                                index: 0,
                                isFirst: true,
                                isLast: false,//this loop for just befor the last label
                                value: value
                        });
                        if (str || str === 0) {
                                renderer.text(
                ...
                for (j = 0; j < numDiv; j += 1) {
                    ...
                    // do the translation
                    while (i --) {
                            length = mathAbs(DivGap * (j + 1)) * trans;
                            Px = CX + (length * mathCos(-(startAngle + (i * xAngle))));
                            Py = CY + (length * mathSin(-(startAngle + (i * xAngle))));
                            //draw the divline
                            numdivPath[j].splice(numdivPath[j].length, 0 , Px, Py);
                        if (i == 0) {
                            //draw the yAxis div labels
                            if (FCconf.showyaxisvalues != 0 && FCconf.showdivlinevalues != '0') {
                                value = yMin + (DivGap * (j + 1));
                                    str = labelFormatterY.call({
                                            index: j + 1,
                                            isFirst: j == 0,
                                            isLast: false,//this loop for just befor the last label
                    ...
                    series.radar.divline[j] = renderer.path(numdivPath[j]).
					attr({
                                                zIndex: 2,
						'stroke': series.yAxis.options.gridLineColor,
						'stroke-width': (parseInt(FCconf.divlinethickness) || 1) + PX
					}).add(radarGroup);
                }
                ...
                ///draw the max label
                  if (FCconf.showyaxisvalues != 0 && FCconf.showlimits != '0') {
                    value = yMax;
                        ...
                        if (str || str === 0) {
                                x1 = CX + (R * mathCos(-(startAngle)));
                                y1 = CY + (R * mathSin(-(startAngle)));
                                renderer.text(
                                        str,
                                        x1,
                                        y1,
		...
		// draw the axis line
		while (i --) {
                        angle = (startAngle + (i * xAngle));
                        Px = CX + (R * mathCos(- angle));
                        Py = CY + (R * mathSin(-angle));
                        ///draw the axis
			axPath.splice(axPath.length, 0 , M, CX, CY, L, Px, Py);
			//draw the border
                        brPath.splice(brPath.length, 0 , Px, Py);
                    if (first) {
                        brPath.push(L);
                        first = false;
                    }
                    ///draw the xaxis labels
                    if (FCconf.showlabels != '0') {
                        value = xCat[i];
                        str = labelFormatterX.call({
                                index: i,
                                isFirst: j == 0,
                                isLast: false,//this loop for just befor the last label
                                value: value
                        ...
                        if (str || str === 0) {
                            ///re calculate the px py quite bigger
                            Px = CX + ((R + 10) * mathCos(- angle));
                            Py = CY + ((R + 10) * mathSin(-angle));
                                renderer.text(
                                        value,
                                        Px,
                                        Py,
                                        labelOptionsX.style,
                                        0,
                                        (angle == 3 * math.PI / 2 || angle == math.PI / 2) ? 'center' :((angle < 3 * math.PI / 2 && angle > math.PI / 2) ? 'right' : 'left')
                                ).add(radarGroup);
                        }
                    }
		}
                ///close the border
                brPath.push('Z');
                series.radar.axis = renderer.path(axPath).
					attr({
                                                zIndex: 2,
						'stroke': convertColor(FCconf.radarspikecolor, FCconf.radarspikealpha || FCconf.radarinlinealpha, series.yAxis.options.gridLineColor),
						'stroke-width': (parseInt(FCconf.radarspikethickness) || 1) + PX
					}).add(radarGroup);
                if (FCconf.showradarborder != '0') {
                    series.radar.border = renderer.path(brPath).
                    attr({
                        'stroke': series.yAxis.options.radarbordercolor,
                        'stroke-width': (parseInt(FCconf.radarborderthickness) || 2) + PX,
                        'fill': series.yAxis.options.radarfillcolor
                    }).add(radarGroup);
                }
	...
	render: function() {
		...
		if (!series.clipRect) {
			...
			if (!chart.clipRect) {
                ...
                ///draw the axis
                if (series.yAxis.DoradarGraph !== true) {
                    series.plotRadarAxis();
                    series.yAxis.DoradarGraph = true;///flag to draw the radar garh for the first series
                }
