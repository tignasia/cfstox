...
if (window.__orphanarticle!==false && window.parent && (window.parent === window || window.parent.location.domain !== window.location.domain)) {
	window.__orphanarticle = location.href.replace(/contents\//i, 'Contents/Index.html?');
}
...
if (typeof jQuery != 'undefined') { 
	...
	$(document).ready(function()
	{
	 ...
	 // slide in out		
		...
		if (window.__orphanarticle) {
			$('body').prepend('<p class="minified-article-notice highlightBlock">You are viewing the standalone version of this article. To view the this article along with the menu, <a href="'+ 
				window.__orphanarticle +'">click here</a>.</p>');
		}
	...
	// automatic conversion of XML to JSON
/*	$(document).ready(function() {
		console.log(window.parent.location.domain === window.location.domain);
		console.log(('abcd://contents/aa/' + location.href).split(/contents\/.+$/i))
		//console.log($('.prettyprint.lang-xml').text());
	}); */
