...
--->
	...
	<cffunction name="RunSystem" description="" access="public" displayname="" output="false" returntype="Any">
		...
		<cfscript>
		// SetUpData()
		// TradeData = session.objects.SystemRunner.RunSystem(SystemName:"SystemName", data:data);
		// return TradeData
		local.strData = session.objects.Utility.QrytoStruct(query:arguments.qryDataOriginal,rownumber:1);
		//dump(local.strData);
		local.TestData = GetBeans(TestData:local.strData);
		local.TestData 					= StructNew(); 		
		local.TestData.Databeans 		= StructNew(); 		
		local.TestData.TrackingBean 	= createObject("component","cfstox.model.TrackingBean").init(local.strData);
		local.TestData.TradeBean 		= createObject("component","cfstox.model.TradeBean").init(local.strData);
		...
		local.TestResults = RunSystem(qryOriginal:arguments.qryDataOriginal,qryHA:arguments.qryDataHA);
	...
	<cffunction name="GetBeans" description="" access= displayname="" output= returntype=>
