/*!
 * FusionCharts Tag Manipulation JavaScript Library
 ...
 *
 *  1.1.1 - 09/08/2010
 *  Removed dependency from InfoSoft Global namespace.
 *
 *  1.1.0 - 30/04/2009:
 ...
 *  54: Fixed a bug found in multilevelpie chart alias
 *  55: Added SSGrid alias
 *
 *  QA47 Fixes
...
var _FCD = FusionChartsDOM; // internal coding variable

/**
 *  Version of this JS [major, minor, revision, build]
 */
_FCD.version = [1, 1, 1, 'release', 58];
...
_FCD.RenderChart = function(index)
{
    ...
    getChartFromId(_FCD.Nodes[index].id);
...
_FCD.processTag = function(tagObj)
{
    ...
    {
		if (newChart.vars) {
			newChart.vars[aVar] = chartVars[aVar];
		}
		else if (newChart.addVariable) {
	...
	if (newChart.vars) {
		newChart.vars['FusionChartsDOM'] = _FCD.version.toString();
	}
    else {
